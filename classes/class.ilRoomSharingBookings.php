...
class ilRoomSharingBookings
{
	...
	public function removeBooking($a_booking_id, $a_seq = false)
	{
		...
		{
			$set = $ilDB->query( 'SELECT seq_id, user_id' . ' FROM rep_robj_xrs_bookings' . ' WHERE id = ' . $ilDB->quote( $a_booking_id, 'integer' ) );
			...
			if ($ilDB->numRows( $set ) > 0)
			{
				...
				if ($row ['user_id'] == $ilUser->getId())
				{
					...
					{
						$ilDB->query( 'DELETE FROM rep_robj_xrs_bookings' . ' WHERE id = ' . $ilDB->quote( $a_booking_id, 'integer' ) );
						$ilDB->query( 'DELETE FROM rep_robj_xrs_book_user' . ' WHERE booking_id = ' . $ilDB->quote( $a_booking_id, 'integer' ) );
					...
					else
					{
						...
						$seq_set = $ilDB->query( 'SELECT id FROM rep_robj_xrs_bookings' . ' WHERE seq = ' . $ilDB->quote( $row ['seq'], 'integer' ) . ' AND pool_id = ' . $ilDB->quote( $pool_id, 'integer' ) );
	...
	public function getList()
	{
		...
		$set = $ilDB->query( 'SELECT *' . ' FROM rep_robj_xrs_bookings' . ' WHERE pool_id = ' . $ilDB->quote( $this->pool_id, 'integer' ) . ' AND user_id = ' . $ilDB->quote( $ilUser->getId(), 'integer' ) . ' AND (date_from >= "' . date( 'Y-m-d H:i:s' ) . '"' . ' OR date_to >= "' . date( 'Y-m-d H:i:s' ) . '")' . ' ORDER BY date_from ASC' );
		$res = array ();
		while ( $row = $ilDB->fetchAssoc( $set ) )
		{
			...
			} else
			{
				...
				$date = $date_from->format( 'd' ) . '.' . ' ' . $lng->txt( 'month_' . $date_from->format( 'm' ) . '_short' ) . ' ' . $date_from->format( 'Y' ) . ',' . ' ' . $date_from->format( 'H:i' );
				$date .= " - ";
				
				// Check whether the date_from differs from the date_to
				if ($date_from->format( 'dmY' ) !== $date_to->format( 'dmY' ))
				{
					$date .= '<br>' . $date_to->format( 'd' ) . '.' . ' ' . $lng->txt( 'month_' . $date_to->format( 'm' ) . '_short' ) . ' ' . $date_to->format( 'Y' ) . ', ';
				}
			...
			$roomSet = $ilDB->query( 'SELECT name FROM rep_robj_xrs_rooms' . ' WHERE id = ' . $ilDB->quote( $row ['room_id'], 'integer' ) );
			...
			$participantSet = $ilDB->query( 'SELECT users.firstname AS firstname,' . ' users.lastname AS lastname, users.login AS login,' . ' users.usr_id AS id FROM rep_robj_xrs_book_user' . ' LEFT JOIN usr_data AS users ON users.usr_id = rep_robj_xrs_book_user.user_id' . ' WHERE booking_id = ' . $ilDB->quote( $row ['id'], 'integer' ) . ' ORDER BY users.lastname, users.firstname ASC' );
			...
			$attributesSet = $ilDB->query( 'SELECT value, attr.name AS name' . ' FROM rep_robj_xrs_book_attr' . ' LEFT JOIN rep_robj_xrs_battr AS attr' . ' ON attr.id = rep_robj_xrs_book_attr.attr_id' . ' WHERE booking_id = ' . $ilDB->quote( $row ['id'], 'integer' ) );
	...
	public function getAdditionalBookingInfos()
	{
		global $ilDB;
		$cols = array ();
		$attributesSet = $ilDB->query( 'SELECT *' . ' FROM rep_robj_xrs_battr' . ' WHERE pool_id = ' . $ilDB->quote( $this->pool_id, 'integer' ) );
		while ( $attributesRow = $ilDB->fetchAssoc( $attributesSet ) )
		{
