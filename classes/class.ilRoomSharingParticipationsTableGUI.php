...
class ilRoomSharingParticipationsTableGUI extends ilTable2GUI
{
	...
	public function __construct($a_parent_obj, $a_parent_cmd, $a_ref_id)
	{
		...
		// command for leaving
		$this->addMultiCommand('showParticipations', 
				$this->lng->txt('rep_robj_xrs_leave'));
	...
	private function addColumns()
	{
		...
		$this->addColumn($this->lng->txt("rep_robj_xrs_person_responsible"), 
				"person_responsible");
	 ...
	 *
	 * @param array $a_set with data to be filled in.
	 */
	public function fillRow($a_set)
	{
		...
		$this->tpl->setVariable('HREF_ROOM', 
				$this->ctrl->getLinkTargetByClass('ilobjroomsharinggui', 
						'showRoom'));
		$this->ctrl->setParameterByClass('ilobjroomsharinggui', 'room_id', '');
		
		$this->tpl->setVariable('TXT_SUBJECT', 
				($a_set['subject'] == null ? '' : $a_set['subject']));
		...
		$this->tpl->setVariable('LINK_ACTION', 
				$this->ctrl->getLinkTarget($this->parent_obj, 
						'showParticipations'));
		$this->tpl->setVariable('LINK_ACTION_TXT', 
				$this->lng->txt('rep_robj_xrs_leave'));
	}
