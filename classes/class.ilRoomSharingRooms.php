...
class ilRoomSharingRooms
{
	...
	public function getList(array $filter = null)
	{
		...
		if ($filter ['attributes'])
		{
			...
			{
				$count = $count + 1;
				$q = 'SELECT room_id FROM rep_robj_xrs_room_attr ra LEFT JOIN rep_robj_xrs_rattr attr ON ra.att_id = attr.id WHERE name = '.
				$ilDB->quote($key, 'text').
				' AND count >= '.$ilDB->quote($value, 'integer').
		...
		$st = $ilDB->prepare('SELECT room.id, name, max_alloc FROM rep_robj_xrs_rooms room WHERE '.
				$ilDB->in("room.id", array_keys($roomsMatchingAttributeFilters)).$where_part.
				' ORDER BY name', $ilDB->addTypesToArray($types, "integer", count($room.ids)));
	...
	protected function getAttributes(array $room_ids)
	{
		...
		$st = $ilDB->prepare('SELECT room_id, att.name, count FROM rep_robj_xrs_room_attr '.
				...
				' ORDER BY room_id, att.name', $ilDB->addTypesToArray($types, "integer", count($room_ids)));
	...
	public function getMaxCountForAttribute($a_room_attribute)
	{
		...
		$valueSet = $ilDB->query('SELECT MAX(count) AS value FROM rep_robj_xrs_room_attr LEFT JOIN rep_robj_xrs_rooms as room
				 ON room.id = rep_robj_xrs_room_attr.room_id WHERE att_id ='.
				$ilDB->quote($attributID, 'integer').
				' AND pool_id ='.$ilDB->quote($this->pool_id, 'integer'));
		$valueRow = $ilDB->fetchAssoc($valueSet);
		$value = $valueRow ['value'];
		return $value;
