...
class ilRoomSharingShowAndEditBookGUI
{
	...
	private function createAndSetFormItems()
	{
		...
		$this->date_from = $bookingData['booking_values'][0]['date_from'];
		$this->date_to = $bookingData['booking_values'][0]['date_to'];
		$this->old_booking_values = $bookingData['booking_values'][0];
		$this->old_attr_values = $bookingData['attr_values'][0];
		$this->old_participants = $bookingData['participants'];
		$form_items = array();
		$form_items[] = $this->createAndSetSubjectTextInput($bookingData['booking_values'][0]);
		$form_items[] = $this->createAndSetCommentTextInput($bookingData['booking_values'][0]);
		$booking_attributes = $this->createAndSetBookingAttributeTextInputs($bookingData['attr_values'][0]);
		$form_items = array_merge($form_items, $booking_attributes);
		$form_items[] = $this->createAndSetTimeRangeInput();
		$form_items[] = $this->createAndSetPublicBookingCheckBox($bookingData['booking_values'][0]);
