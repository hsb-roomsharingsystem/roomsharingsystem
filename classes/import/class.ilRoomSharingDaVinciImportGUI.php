...
class ilRoomSharingDaVinciImportGUI
{
	...
	public function importObject()
	{
		...
		if ($form->checkInput())
		{
			$import = new ilRoomSharingDaVinciImport($this->parent_obj, $this->pool_id,
	...
	protected function initImportForm()
	{
		...
		$importOption1field = new ilCheckboxInputGUI($this->lng->txt("rep_robj_xrs_daVinci_import_bookings"),
			'import_bookings');
		$import_form->addItem($importOption1field);

		$importOption2field = new ilCheckboxInputGUI($this->lng->txt("rep_robj_xrs_daVinci_import_rooms"),
			'import_rooms');
		$import_form->addItem($importOption2field);

		$default_cap = new ilRoomSharingNumberInputGUI($this->lng->txt("rep_robj_xrs_daVinci_import_default_cap"),
			"default_cap");
		...
		$import_form->addCommandButton($this->lng->txt("rep_robj_xrs_daVinci_import_button"),
			$this->lng->txt("import"));
