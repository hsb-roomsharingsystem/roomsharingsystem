...
/**
 * Class ilRoomSharingRoomGUI. The caller must have implemented method getPoolId().
...
//         $this->tpl->setContent($toolbar->getHTML() . $this->form_gui->getHTML());
     ...
     * Build property form.
     *
     * @return ilPropertyFormGUI for an room.
     */
    function initForm ($a_mode = "show")
    {
        ...
        if (isset($this->room_id) && ! empty($this->room_id)) {
        ...
        if ($a_mode == "edit" || $a_mode == "show") {
            ...
            $post->setTitle($lng->txt("rep_robj_xrs_room_attributes"));
            $form_gui->addItem($post);
            $attributes = $this->room_obj->getAttributes();
            foreach ($attributes as $attrs => $attr) {
