...
class ilRoomSharingBookGUI
{
	 ...
	 */
	function __construct(ilObjRoomSharingGUI $a_parent_obj, $a_room_id = null, $a_date_from = "", $a_date_to = "")
	{
	...
	function initForm()
	{
		...
		$form->setTitle($lng->txt('rep_robj_xrs_room_book') . ': ' .
				$lng->txt('rep_robj_xrs_room') . ' ' . $this->ilRoomSharingRooms->
						getRoomName((empty($this->room_id)) ?
	 ...
	 * Sets the room ID
	 *
	 * @param $a_room_id Room Id which should be set
	 */
