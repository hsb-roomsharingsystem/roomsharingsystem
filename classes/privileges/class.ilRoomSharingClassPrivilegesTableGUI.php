...
class ilRoomSharingClassPrivilegesTableGUI extends ilTable2GUI
{
	...
	private function addTooltips()
	{
		...
		foreach ($classes as $class)
		{
			...
			ilTooltipGUI::addTooltip("thc_" . $this->getId() . "_" . $cnt,
				"<pre>" . $this->lng->txt("rep_robj_xrs_class") . ": " . $class["name"] . "&#13;&#10;"
				. $this->lng->txt("rep_robj_xrs_privileges_role_assignment") . ": " . $role_text . "&#13;&#10;" . $this->lng->txt("rep_robj_xrs_class_priority") . ": " . $class["priority"] . "</pre>",
	...
	private function createTitle($a_class_set)
	{
		...
		return '<a class="tblheader" href="' . $this->ctrl->getLinkTargetByClass("ilroomsharingclassgui",
				"") . '" >' . $table_head . "</a>";
	}
